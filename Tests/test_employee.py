...
class TestAddEmployee(WebDriverWrapper):
    @pytest.mark.parametrize('useremail,password,upload_number,expected_error',
                             data_source.test_invalid_profile_upload_data)
    def test_invalid_profile_upload(self, useremail, password, upload_number, expected_error):
        self.driver.find_element(By.ID, "session_key").send_keys(useremail)
